...
/************************************6*7************************************/
const unsigned char string6X7Code[][6] =
	{
...
{
	return HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, data, len, 5);
}

void writeOledCmd(unsigned char cmd)
{
	HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x00, I2C_MEMADD_SIZE_8BIT, &cmd, 1, 1);
}

void writeOledData(unsigned char dat)
{
	HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, &dat, 1, 1);
}

void setOledPoint(int x, int y)
{
	writeOledCmd(0xb0 + y);
	writeOledCmd(((x & 0xf0) >> 4) | 0x10);
	writeOledCmd((x & 0x0f));
}

void openOledDis(void)
{
	writeOledCmd(0X8D); // SET DCDC
	writeOledCmd(0X14); // DCDC ON
...
void WS_OLED_Dis_String(int x, int y, char *string, char disMode)
{
	...
	while (string[index] > 0)
	{
		...
		case 0:
			...
			{
				setOledPoint(x, y);
				writeOledBuf(0x40, (unsigned char *)string6X7Code[c - ' '], 6);
				x += 6;
				if (x > 128 - 6)
				{
					x = 0;
		...
		default:
			if (c < 0x80) // ASCII
			{
				c -= ' ';
				setOledPoint(x, y);
				writeOledBuf(0x40, (unsigned char *)string8X16Code[c], 8);
				setOledPoint(x, y + 1);
				writeOledBuf(0x40, (unsigned char *)string8X16Code[c] + 8, 8);
			}
			// else
			// {
			//     p_buf = getGb16x16Code(string[index],string[index+1]);
			//     index ++;
			// 	setOledPoint(x,y);
			// 	writeOledBuf(0x40,p_buf,16);
			// 	setOledPoint(x,y+1);
			// 	writeOledBuf(0x40,p_buf+16,16);
			// 	x += 8;
			// }
...
void WS_OLED_Printf(int x, int y, char disMode, char *p, ...)
{
	...
	vsprintf(buf, p, ap);
	va_end(ap);
	WS_OLED_Dis_String(x, y, buf, disMode);
}
...
void WS_OLED_Init(void)
{
	...
	writeOledCmd(0xAE); //--display off
	writeOledCmd(0x02); //---set low column address
	...
	writeOledCmd(0xD8); // set area color mode off
	writeOledCmd(0x05); //

	writeOledCmd(0xD9); // Set Pre-Charge Period
	writeOledCmd(0xF1); //

	writeOledCmd(0xDA); // set com pin configuartion
	writeOledCmd(0x12); //
